...
function mapClick(evt) {
    ...
    if (evt.clicksource === 'pin' && evt.annotation != selectedPin) {
        evt.annotation.title = ""; // hides the original annotation
        showPopView(evt);
...
function closePopView(evt) {
    ...
    popView.hide();
    if (selectedPin) {
        evt.source.deselectAnnotation(selectedPin);
        selectedPin.title = ".";
        selectedPin.setImage(selectedPin.pinImage);
        selectedPin = null;
    }
...
function showPopView(evt) {
    ...
    popLabel.text = evt.annotation.customTitle;
    popView.center = {
        x: point.x,
        y: (point.y - (popView.height / 2) - 20)
    };
